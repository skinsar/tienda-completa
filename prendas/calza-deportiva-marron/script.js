...
const productoId = "qhpihcE7MptmZ5zqc17n"; // <--- CAMBIAR ESTO PARA CADA PRODUCTO
...
// Evento cuando se cambia el color (VERSIÓN ACTUALIZADA)
selectorColor.addEventListener('change', () => {
    const talleSeleccionado = selectorTalle.value;
    const colorSeleccionado = selectorColor.value;

    // Por defecto, deshabilitamos el botón y limpiamos el texto
    btnAgregarCarrito.disabled = true;
    stockDisponible.textContent = ''; 
    ...
    if (talleSeleccionado && colorSeleccionado && productoData) {
        ...
        // --- INICIO DEL CAMBIO ---
        // En lugar de mostrar el número de stock, mostramos un estado.
        if (stock > 0) {
            stockDisponible.textContent = 'Disponible';
            stockDisponible.style.color = 'green'; // Le damos un color verde para que se note
            btnAgregarCarrito.disabled = false; // Activamos el botón porque hay stock
        } else {
            stockDisponible.textContent = 'Sin stock';
            stockDisponible.style.color = 'red'; // Le damos un color rojo
        }
        // --- FIN DEL CAMBIO ---
